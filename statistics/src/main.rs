...
fn process_gene_expresssion_data_headers(
    ...
    let STP: Vec<String> = SID
        .iter()
        .map(|k| SIF.get(&k.to_string()).unwrap())
        .cloned()
        .collect();
    (SID, STP, indices)
}
...
fn process_gene_expresssion_data(thisline: &String, indices: &Vec<usize>) -> (Vec<f64>, String) {
    let gene_expression_measure_values: Vec<String> =
        thisline.split("\t").map(|s| s.to_owned()).collect();
    let gene_expression_measures = indices
        .iter()
        .map(|&i| gene_expression_measure_values[i].clone());
    let gene_expression_measures: Vec<f64> = gene_expression_measures
        .map(|x| x.parse().unwrap())
        .collect();
    let gene_identifiers = vec![
        gene_expression_measure_values[0].clone(),
        gene_expression_measure_values[1].clone(),
    ]
    .join(";");
...
fn filter_out_relevant_columns(
    samples: &Vec<usize>,
...
fn mean_of_samples(
    ...
    Msamples.mean_axis(Axis(1)).unwrap()
}
...
fn process_file(filename: &Path) -> io::Result<HashMap<String, String>> {
    ...
    'linereading: for line in BufReader::new(file).lines() {
        ...
        if within_dataset_table && !gene_expression_headers {
            ...
            let (gene_expression_measures, gene_identifiers) =
                process_gene_expresssion_data(&thisline, &indices);
    ...
    let gene_expression_measures_matrix =
        Array::from_shape_vec((22283, 127), gene_expression_measures_vec).unwrap();
...
// practical-machine-learning-with-rust/statistics/data/GDS1615_full.soft
// practical-machine-learning-with-rust/statistics$ cargo run
// Output:
// z scores mean 0.042918212228268235
// z scores mean 3.5369539066322027, shape=[], strides=[], layout=CF (0x3), const ndim=0
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // This dataset contains analysis of peripheral blood
    // mononuclear cells (PBMCs) from 59 Crohn’s disease patients
    // and 26 ulcerative colitis (UC) patients. There are 22,283 samples.
    // In this section, we will consider the mean expression levels in the two groups.
    ...
    let filename = Path::new("data/GDS1615_full.soft");
